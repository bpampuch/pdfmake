...
class TableProcessor {
	...
	beginTable(writer) {
		...
		function prepareRowSpanData() {
			...
			for (let i = 0, l = self.tableNode.table.body[0].length; i < l; i++) {
				const paddings = self.layout.paddingLeft(i, self.tableNode) + self.layout.paddingRight(i, self.tableNode);
				const lBorder = self.layout.vLineWidth(i, self.tableNode);
	...
	onRowBreak(rowIndex, writer) {
		const self = this;
		return function () {
			const offset = self.rowPaddingTop + (!self.headerRows ? self.topLineWidth : 0);
	...
	drawHorizontalLine(lineIndex, writer, overrideY) {
		const lineWidth = this.layout.hLineWidth(lineIndex, this.tableNode);
		if (lineWidth) {
			const style = this.layout.hLineStyle(lineIndex, this.tableNode);
			let dash;
			if (style && style.dash) {
				dash = style.dash;
			}
	...
	drawVerticalLine(x, y0, y1, vLineIndex, writer) {
		const width = this.layout.vLineWidth(vLineIndex, this.tableNode);
		if (width === 0) {
			return;
		}
		const style = this.layout.vLineStyle(vLineIndex, this.tableNode);
		let dash;
		if (style && style.dash) {
			dash = style.dash;
		}
	...
	endRow(rowIndex, writer, pageBreaks) {
		...
		const skipOrphanePadding = (ys[0].y1 - ys[0].y0 === this.rowPaddingTop);
		for (let yi = (skipOrphanePadding ? 1 : 0), yl = ys.length; yi < yl; yi++) {
			...
			for (i = 0, l = xs.length; i < l; i++) {
				let leftBorder = false, rightBorder = false;
				...
				if (i < l - 1) {
					...
					if (fillColor) {
						const wBorder = (leftBorder || rightBorder) ? this.layout.vLineWidth(colIndex, this.tableNode) : 0;
						const xf = xs[i].x + wBorder;
						const yf = this.dontBreakRows ? y1 : y1 - hzLineOffset;
						writer.addVector({
							type: 'rect',
							x: xf,
							y: yf,
		...
		const row = this.tableNode.table.body[rowIndex];
		...
		function getLineXs() {
			...
			for (let i = 0, l = self.tableNode.table.body[rowIndex].length; i < l; i++) {
				if (!cols) {
					...
					const item = self.tableNode.table.body[rowIndex][i];
