...
function getPageSize(currentPage, newPageOrientation) {
...
class DocumentContext {
	...
	beginColumn(width, offset, endingCell) {
		const saved = this.snapshots[this.snapshots.length - 1];
	...
	addPage(pageSize) {
		const page = {items: [], pageSize: pageSize};
	...
	getCurrentPosition() {
		const pageSize = this.getCurrentPage().pageSize;
		const innerHeight = pageSize.height - this.pageMargins.top - this.pageMargins.bottom;
		const innerWidth = pageSize.width - this.pageMargins.left - this.pageMargins.right;
