...
const qrEncoder = require('./qrEnc.js');
...
class DocMeasure {
		...
		function getNodeMargin() {
			...
			function flattenStyleArray(styleArray) {
				...
				for (let i = styleArray.length - 1; i >= 0; i--) {
					const styleName = styleArray[i];
					const style = self.styleStack.styleDictionary[styleName];
			...
			if (node.style) {
				const styleArray = isArray(node.style) ? node.style : [node.style];
	...
	convertIfBase64Image(node) {
		if (/^data:image\/(jpeg|jpg|png);base64,/.test(node.image)) {
			const label = '$$pdfmake$$' + this.autoImageIndex++;
	...
	measureImage(node) {
		...
		const imageSize = this.imageMeasure.measureImage(node.image);
	
		if (node.fit) {
			const factor = (imageSize.width / imageSize.height > node.fit[0] / node.fit[1]) ? node.fit[0] / imageSize.width : node.fit[1] / imageSize.height;
	...
	measureLeaf(node) {
		...
		const data = this.textTools.buildInlines(node.text, styleStack);
	...
	measureToc(node) {
		...
		const body = [];
		const textStyle = node.toc.textStyle || {};
		const numberStyle = node.toc.numberStyle || textStyle;
		const textMargin = node.toc.textMargin || [0, 0, 0, 0];
		for (let i = 0, l = node.toc._items.length; i < l; i++) {
			const item = node.toc._items[i];
			const lineStyle = item._textNodeRef.tocStyle || textStyle;
			const lineMargin = item._textNodeRef.tocMargin || textMargin;
			body.push([
	...
	buildUnorderedMarker(styleStack, gapSize, type) {
		...
		let marker;
		const color = styleStack.getProperty('markerColor') || styleStack.getProperty('color') || 'black';
	...
	buildOrderedMarker(counter, styleStack, type, separator) {
		...
		const textArray = {text: counterText};
		const markerColor = styleStack.getProperty('markerColor');
		if (markerColor) {
	...
	measureUnorderedList(node) {
		...
		for (let i = 0, l = items.length; i < l; i++) {
			const item = items[i] = this.measureNode(items[i]);
	...
	measureColumns(node) {
		...
		const numGaps = (columns.length > 0) ? (columns.length - 1) : 0;
	...
	measureTable(node) {
		...
		const measures = ColumnCalculator.measureMinMax(node.table.widths);
		...
		function getOffsets(layout) {
			...
			for (let i = 0, l = node.table.widths.length; i < l; i++) {
				const lOffset = prevRightPadding + layout.vLineWidth(i, node) + layout.paddingLeft(i, node);
		...
		function extendWidthsForColSpans() {
			...
			for (let i = 0, l = colSpans.length; i < l; i++) {
				...
				const currentMinMax = getMinMax(span.col, span.span, node._offsets);
				const minDifference = span.minWidth - currentMinMax.minWidth;
				const maxDifference = span.maxWidth - currentMinMax.maxWidth;
