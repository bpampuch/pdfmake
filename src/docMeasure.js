...
DocMeasure.prototype.measureNode = function(node, callback) {
  ...
  return this.styleStack.asyncAuto(node, function(cb) {
    ...
    if (node.columns) {
      self.measureColumns(node, function(err, node) {
        if (err) {
          return cb(err);
        }
        cb(null, extendMargins(node));
      });
    } else if (node.stack) {
      self.measureVerticalContainer(node, function(err, node) {
        if (err) {
          return cb(err);
        }
        cb(null, extendMargins(node));
      ...
      self.measureUnorderedList(node, function(err, node) {
        if (err) {
          return cb(err);
        }
        cb(null, extendMargins(node));
      ...
      self.measureOrderedList(node, function(err, node) {
        if (err) {
          return cb(err);
        }
        cb(null, extendMargins(node));
      ...
      self.measureTable(node, function(err, node) {
        if (err) {
          return cb(err);
        }
        cb(null, extendMargins(node));
      ...
      self.measureLeaf(node, function(err, node) {
        if (err) {
          return cb(err);
        }
        cb(null, extendMargins(node));
      ...
      self.measureToc(node, function(err, node) {
        if (err) {
          return cb(err);
        }
        cb(null, extendMargins(node));
      ...
      self.measureImage(node, function(err, node) {
        if (err) {
          return cb(err);
        }
        cb(null, extendMargins(node));
      ...
      self.measureCanvas(node, function(err, node) {
        if (err) {
          return cb(err);
        }
        cb(null, extendMargins(node));
      ...
      self.measureQr(node, function(err, node) {
        if (err) {
          return cb(err);
        }
        cb(null, extendMargins(node));
      ...
      cb(new Error('Unrecognized document structure: ' + JSON.stringify(node, fontStringify)));
    }
  }, callback);
...
DocMeasure.prototype.measureImage = function(node, cb) {
  var self = this;
  if (this.images) {
    this.convertIfBase64Image(node);
  }

  this.imageMeasure.measureImage(node.image, function(err, imageSize) {
    if (err) {
      return cb(err);
    }
    ...
    node._alignment = self.styleStack.getProperty('alignment');
    return cb(null, node);
  });
};

DocMeasure.prototype.measureLeaf = function(node, cb) {
...
DocMeasure.prototype.measureToc = function(node, cb) {
  if (node.toc.title) {
  ...
  this.measureNode(node.toc._table, function(err, tableSize) {
    if (err) {
      return cb(err);
    }
    node.toc._table = tableSize;
    return cb(null, node);
  });
};

DocMeasure.prototype.measureVerticalContainer = function(node, cb) {
  var self = this;
  ...
  var i = 0;
  (function loop() {
    self.measureNode(items[i], function(err, item) {
      if (err) {
        return cb(err);
      }
      items[i] = item;
...
DocMeasure.prototype.measureUnorderedList = function(node, cb) {
  var self = this;
  ...
  (function loop() {
    self.measureNode(items[i], function(err, item) {
      ...
      if (!item.ol && !item.ul) {
        item.listMarker = self.buildUnorderedMarker(style, node._gapSize, item.listType || node.type);
      }
...
DocMeasure.prototype.measureOrderedList = function(node, cb) {
  var self = this;
  var style = this.styleStack.clone();
  var items = node.ol;
  ...
  (function loop() {
    self.measureNode(items[i], function(err, item) {
      ...
      if (!item.ol && !item.ul) {
        item.listMarker = self.buildOrderedMarker(item.counter || counter, style, item.listType || node.type, node.separator);
...
DocMeasure.prototype.measureColumns = function(node, cb) {
  var self = this;
  ...
  var i = 0;
  (function loop() {
    self.measureNode(columns[i], function(err, column) {
      if (err) {
        return cb(err);
      }
      columns[i] = column;
...
DocMeasure.prototype.measureTable = function(node, cb) {
  ...
  var col = 0;
  var row = 0;
  var cols = node.table.body[0].length - 1;
  var rows = node.table.body.length - 1;
  ...
  (function colLoop() {
    ...
    (function rowLoop() {
      ...
      if (data === undefined) {
        ...
        return cb(new Error('Malformed table row, a cell is undefined.'));
      }
      ...
      if (!data._span) {
        return self.styleStack.asyncAuto(data, measureCb(self, data), function(err, data) {
          if (err) {
            return cb(err);
          }
          rowData[col] = data;
  ...
  function loop(colLoop, rowLoop) {
    if (row < rows) {
      ++row;
      setTimeout(rowLoop, 0);
    } else if (col < cols) {
      row = 0;
      ++col;
      setTimeout(colLoop, 0);
    } else {
  ...
  function measureCb(_this, data) {
    return function(cb) {
      ...
      return _this.measureNode(data, cb);
    };
  }
...
DocMeasure.prototype.measureCanvas = function(node, cb) {
  var w = 0, h = 0;
...
DocMeasure.prototype.measureQr = function(node, cb) {
