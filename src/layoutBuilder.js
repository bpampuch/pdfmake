...
LayoutBuilder.prototype.layoutDocument = function(docStructure, fontProvider, styleDictionary, defaultStyle, background, header, footer, images, watermark, pageBreakBeforeFct, cb) {
  ...
  (function While() {
    self.tryLayoutDocument(docStructure, fontProvider, styleDictionary, defaultStyle, background, header, footer, images, watermark, function(err, result) {
      if (err) {
        return cb(err);
      }
      if (addPageBreaksIfNecessary(result.linearNodeList, result.pages)) {
        resetXYs(result);
        return setTimeout(While, 0);
      }
      cb(null, result.pages);
    });
  })();
};

LayoutBuilder.prototype.tryLayoutDocument = function(docStructure, fontProvider, styleDictionary, defaultStyle, background, header, footer, images, watermark, cb) {
  var self = this;
  this.linearNodeList = [];
  this.processed = 0;
  ...
  docStructure = this.docMeasure.measureDocument(docStructure, function(err, docStructure) {
    ...
    self.writer = new PageElementWriter(
      new DocumentContext(self.pageSize, self.pageMargins), self.tracker);

    self.writer.context().tracker.startTracking('pageAdded', function() {
...
LayoutBuilder.prototype.addBackground = function(background) {
  ...
  if (pageBackground) {
    ...
    this.docMeasure.measureDocument(pageBackground, function(err, measureDocument) {
      ...
      self.processNode(measureDocument);
      self.writer.commitUnbreakableBlock(0, 0);
    });
  }
};
