...
describe('Integration test: tables with hardmode', function() {
  const lorem = "Lorem ipsum dolor sit amet";

  const fixtureTableOfCellSize = (size) => {
    return {
      content: {
        table: {
          heights: [size, size, size],
  ...
  var testHelperinHardMode = new integrationTestHelper(true);

  it('throws an error', function() {
    assert.throws(() => testHelperinHardMode.renderPages('A6', fixtureTableOfCellSize(50)));
  });

  it('doesn\'t throws an error', function() {
    assert.ok(() => testHelperinHardMode.renderPages('A6', fixtureTableOfCellSize(60)));
  });
